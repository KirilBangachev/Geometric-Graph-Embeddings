                ...
                if G(i,j)>=0
                    ...
                    for u = 1:d 
                        ...
                        if (dotij>-1 + eps) & (dotij<1 - eps) %To avoid numerical issues
                            egrad(:,d*(i-1)+u) = egrad(:,d*(i-1)+u)-(2/sqrt(1 - dotij^2))*beta.*( ...
                                G(i,j)-sigma_val).*Y(:,d*(j-1)+u);
                        end
                    end
                end
            end
                ...
                if G(i,j)>=0
                    ...
                    for u = 1:d 
                        ...
                        if (dotij>-1 + eps) & (dotij<1 - eps) %To avoid numerical issues
                            ...
                            ehessv(:,d*(i-1)+u) = ehessv(:,d*(i-1)+u)-2*crossterms(1,u)*dotij*beta*( ...
                                (G(i,j)-sigma_val)/(sqrt(1 - dotij^2)^3)).*Y(:,d*(j-1)+u);

                            % Differentiate V part of Euclidean Grad
                            ehessv(:,d*(i-1)+u) = ehessv(:,d*(i-1)+u)-(2/sqrt(1 - dotij^2))*beta.*( ...
                                G(i,j)-sigma_val).*V(:,d*(j-1)+u);;
                        end
                    end
                end
            end
